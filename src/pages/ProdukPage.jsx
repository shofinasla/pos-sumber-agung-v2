...
export function ProdukPage() {
  ...
  const [isClearBarcodesModalOpen, setIsClearBarcodesModalOpen] = useState(false);
  const [clearingBarcodesLoading, setClearingBarcodesLoading] = useState(false);
  ...
  const handleExecuteClearBarcodes = async () => {
    setClearingBarcodesLoading(true);
    const res = await productService.clearAllBarcodes();
    setClearingBarcodesLoading(false);
    setIsClearBarcodesModalOpen(false);

    if (res.error) {
      showToast(res.error.message || 'Gagal mengosongkan barcode produk.', 'error');
    } else {
      showToast('Semua data barcode produk berhasil dikosongkan. Siap untuk import ulang tanpa duplikat.', 'success');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-2xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            type="button"
            onClick={() => setIsClearBarcodesModalOpen(true)}
            title="Kosongkan semua barcode yang terdaftar pada produk agar tidak terdeteksi duplikat"
            className="px-3.5 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded-2xl border border-rose-200 flex items-center space-x-1.5 transition cursor-pointer"
          >
            <RotateCcw className="w-3.5 h-3.5 text-rose-600" />
            <span>Kosongkan Semua Barcode</span>
          </button>
      ...
      {/* Clear All Barcodes Confirm Modal */}
      <ConfirmModal
        isOpen={isClearBarcodesModalOpen}
        onClose={() => setIsClearBarcodesModalOpen(false)}
        onConfirm={handleExecuteClearBarcodes}
        title="Kosongkan Semua Barcode Produk?"
        message="Apakah Anda yakin ingin menghapus / mengosongkan semua barcode produk di database? Tindakan ini mencegah deteksi duplikat saat Anda mengimpor ulang data CSV atau menginput barcode baru. Data nama produk, harga, dan stok tetap aman."
        confirmText="Ya, Kosongkan Semua Barcode"
        isDanger={true}
        loading={clearingBarcodesLoading}
      />
